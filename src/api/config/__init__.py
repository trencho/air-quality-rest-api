...
def create_app():
    ...
    # Comment these 5 lines for initializing mongodb connection when running app in debug mode
    app.config['MONGO_URI'] = (
            'mongodb://' + os.environ.get(mongo_db_user_name_env_value) + ':'
            + os.environ.get(mongo_db_user_pass_env_value) + '@' + os.environ.get(mongo_db_host_env_value) + ':'
            + os.environ.get(mongo_db_port_env_value) + '/' + os.environ.get(mongo_db_name_env_value))
    mongo.init_app(app)

...
@history_blueprint.get(
    "/cities/<string:city_name>/sensors/<string:sensor_id>/history/<string:data_type>/",
...
@history_blueprint.get(
    "/coordinates/<float:latitude>,<float:longitude>/history/<string:data_type>/",
...
def fetch_coordinates_history(
    ...
    return return_historical_data(
        sensor["cityName"], sensor, data_type, start_time, end_time
    )
...
def retrieve_history_timestamps() -> tuple[int, int] | tuple[Response, int]:
    ...
    start_time = request.args.get(
        "start_time", default=current_timestamp - week_in_seconds, type=int
    )

...
def fetch_plots_predictions(city_name: str, sensor_id: str, pollutant: PollutantType) \
...
def fetch_plots_errors(city_name: str, sensor_id: str, pollutant: PollutantType, error_type: ErrorType) \
        -> Response | tuple[Response, int]:
    if check_city(city_name) is None:
        return jsonify(
            error_message="Cannot return plot because the city is not found or is invalid."), HTTP_404_NOT_FOUND
    ...
    image_path = RESULTS_ERRORS_PLOTS_PATH / city_name / sensor_id / pollutant / f"{error_type.value}.png"
    if not image_path.exists():
        return jsonify(error_message="Cannot return plot because it does not exist."), HTTP_404_NOT_FOUND

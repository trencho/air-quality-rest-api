...
@train_blueprint.get(
    "/cities/<string:city_name>/sensors/<string:sensor_id>/train/",
    endpoint="train_city_sensor",
)
...
def train_data(
    ...
    if (city := check_city(city_name)) is None:
        return (
            jsonify(
                error_message="Data cannot be trained because the city is not found or is invalid."
            ),
            HTTP_404_NOT_FOUND,
        )

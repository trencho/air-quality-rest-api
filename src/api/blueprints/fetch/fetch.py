...
@fetch_blueprint.get(
    "/cities/<string:city_name>/sensors/<string:sensor_id>/fetch/",
    endpoint="fetch_city_sensor",
)
...
def fetch_data(
    ...
    if city_name is None:
        ...
        return jsonify(
            success="Fetched weather and pollution data from the external APIs for all cities."
        )

    if (city := check_city(city_name)) is None:
        return (
            jsonify(
                error_message="Cannot fetch data because the city is not found or is invalid."
            ),
            HTTP_400_BAD_REQUEST,
        )

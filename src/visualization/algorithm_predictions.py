...
def draw_predictions(city: dict, sensor: dict, pollutant: str) -> None:
    ...
    fig, ax = pyplot.subplots(
        1, 1, figsize=PLOT_PARAMS["figsize"], dpi=PLOT_PARAMS["dpi"]
    )
    ...
    ax.set_ylabel(
        f"{POLLUTANTS[pollutant]} values", fontsize=PLOT_PARAMS["ylabel_fontsize"]
    )
    ax.tick_params(axis="y", rotation=0)
    ax.set_title(
        f"{city['siteName']} - {sensor['description']}",

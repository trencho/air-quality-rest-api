...
def draw_errors():
    ...
    for station in stations:
        for pollutant in pollutants:
            if not os.path.exists(RESULTS_ERRORS_PATH + '/data/' + station + '/' + pollutant):
                continue
            for error_type in error_types:
                ...
                ax.vlines(x=dataframe_algorithms.index, ymin=0, ymax=dataframe_algorithms[pollutant],
                          color='firebrick',
                ...
                ax.set_title(station, fontdict={'size': 22})
                ...
                fig.tight_layout()
                if not os.path.exists(RESULTS_ERRORS_PATH + '/plots/' + station + '/' + pollutant):
                    os.makedirs(RESULTS_ERRORS_PATH + '/plots/' + station + '/' + pollutant)
                plt.savefig(
                    RESULTS_ERRORS_PATH + '/plots/' + station + '/' + pollutant + '/' + error_type + '.png',
                    bbox_inches='tight')
